...
public class Http11Processor extends AbstractProcessor {
    ...
    private void prepareRequestProtocol() {
        ...
        if (protocolMB.equals(Constants.HTTP_11)) {
            ...
            // HTTP/0.9 is not supported. Reject unconditionally: HTTP/0.9 carries
            // no headers and cannot satisfy authentication or authorization controls.
            http11 = false;
            // 400 - Bad Request
            response.setStatus(400);
            setErrorState(ErrorState.CLOSE_CLEAN, null);
            if (log.isDebugEnabled()) {
                log.debug(sm.getString("http11processor.request.invalidHttp09Method"));
            }
        } else {
